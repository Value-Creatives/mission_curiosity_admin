...
import { getRequestCall, deleteRequestCall } from "../../services/requestcall.service";
...
function RequestCall() {
    ...
    const onDeleteClick = async(row) => {
        ...
        if (confirm) {
            let { data:res } = await deleteRequestCall(row);
            if(res){
                getRequestCall();
    ...
    let requestGget = async()=>{
        try{
            let { data:res } = await getRequestCall();
            if(res){
                let data = res?.data;
    ...
    const category_columns = [
        ...
        // },
        {
            ...
            cell: (row) => <ActionIcon isRedirected={true} editPath={`/RequestCall/ViewRequest?id=${row?._id}`} onDeleteClick={() => onDeleteClick(row?._id)} deletePath="/RequestCall" remove edit Uniquekey={row.id} />,
        },
    ];
