...
import { BANNERGet, SetBANNERObj, BANNERDelete } from "../../../redux/actions/Banner/Banner.actions";
...
function Banners() {
  ...
  const brand_columns = [
    ...
    },
    {
      ...
      button: true,
      width: "20%",
      // cell: (row) => <CustomButton greenBtn noIcon btnName= path={"/Location/View-Testimonial"} />,
      cell: (row) => <CustomButton greenBtn btnName={`${row.status == "APPROVED" ? "Active" : "InActive"}`} />,
    },
    {
      name: "Action",
      width: "20%",
      cell: (row) => <ActionIcon isRedirected={true} onEditClick={() => handleEdit(row)} editPath="/Banners/Banner-Create" onDeleteClick={() => handleDelete(row)} deletePath="/Banners" remove edit Uniquekey={row.id} />,
    },
  ];
