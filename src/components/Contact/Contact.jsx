...
import { getContact, deleteContact } from "../../services/contact.service";
...
function Contact() {
    ...
    const onDeleteClick = async (row) => {
        ...
        if (confirm) {
            let { data: res } = await deleteContact(row);
            if (res) {
                contactget();
    ...
    let contactget = async () => {
        try {
            let { data: res } = await getContact();
            if (res) {
                let data = res?.data;
    ...
    const category_columns = [
        ...
        },
        {
            name: "Email",
            maxWidth: "211px",
            selector: (row) => row?.queryEmail,
        },
        {
            name: "Phone",
            maxWidth: "211px",
            selector: (row) => row?.queryPhone,
        },
        {
            name: "School",
            maxWidth: "211px",
            selector: (row) => row?.querySchool,
        },
        {
            name: "Message",
            maxWidth: "211px",
        ...
        // },
        {
            ...
            cell: (row) => <ActionIcon isRedirected={true} editPath={`/Contact/ViewContact?id=${row?._id}`} onDeleteClick={() => onDeleteClick(row?._id)} deletePath="/Contact" remove edit Uniquekey={row.id} />,
        },
    ];
